...
contract StorageHandler{
    ...
    // New Variable
    function getAirdrop(address account)internal view returns(uint256){
        _storageERC20.getUint(keccak256(abi.encodePacked("airdropAmount",account)));
    }
    ...
    // New Variable
    function setAirdropAmount(address account, uint256 amount)public{
        _storageERC20.setUint(keccak256(abi.encodePacked("airdropAmount",account)),amount);
    }
}